...
public class RecordActivity extends Activity {
    ...
    public void start(View v) {
        ...
        if(d != 0) {
            Record.getInstance(getApplicationContext()).start(d, l);
        } else {
    ...
    public void pause(View v) {
        if (Record.getInstance(getApplicationContext()).isRecording()) {
            Record.getInstance(getApplicationContext()).pause();
            pause.setText("Resume");
        } else {
            Record.getInstance(getApplicationContext()).resume();
            pause.setText("Pause");
        }
    }

    public void stop(View v) {
        Record.getInstance(getApplicationContext()).stop();
    }
}
